...
def conditionMake(data, aggr_df, mode, leafType, lvl, condition, prevBcnt, prevDcnt, ratDf, exceptCon):
    ...
    tmpFinal = tmpFinal.assign(bcntDiff=lambda x: prevBcnt - x.bcnt)
    tmpFinal = tmpFinal.assign(dcntDiff=lambda x: prevDcnt - x.dcnt)
    tmpFinal = tmpFinal.assign(diffRat=lambda x: x.bcntDiff / x.dcntDiff)

    x = tmpFinal[(tmpFinal['diffRat'] > 0.8) & (tmpFinal['diffRat'] < 1.6)].sort_values('dcntDiff',ascending=False)

    try:
        if leafType == 'A':
            # 전 레벨 대비 지우는 비율(bcnt/dcnt)가 0.8에서 2 사이인 녀석 중에 dcnt를 가장 많이 지우는 조건
            tmpFinal = tmpFinal[(tmpFinal['diffRat'] > 0.8) & (tmpFinal['diffRat'] < 4) & (tmpFinal['dcnt'] >= prevDcnt * 0.5)].sort_values('bvsd',
                                                                                                     ascending=False)
        elif leafType == 'B':
            # 이전 레벨 대비 dcnt를 20% 이상 감소시키고 이전 레벨 비율보다 좋아지게 하는 조건
            tmpFinal = tmpFinal[(tmpFinal['diffRat'] > 0.6) & (tmpFinal['dcnt'] >= prevDcnt * 0.3)].sort_values('dcntDiff', ascending=False)
        elif leafType == 'C':
            # 전 레벨 대비 지우는 비율(bcnt/dcnt)가 0.8에서 3 사이인 녀석 중에 dcnt를 가장 많이 지우는 조건
            tmpFinal = tmpFinal[(tmpFinal['diffRat'] > 0.1) & (tmpFinal['dcnt'] >= prevDcnt * 0.5)].sort_values('dcntDiff',
                                                                                                     ascending=False)
        elif leafType == 'D':
            ...
            if lvl == 1 or lvl == 2 or lvl == 3:
                ...
                # 이전 레벨 대비 dcnt를 10% 이상 감소시키고 이전 레벨 비율보다 좋아지게 하는 조건
                tmpFinal = tmpFinal[(tmpFinal['dcntDiff'] > prevDcnt * 0.1)].sort_values('bvsd', ascending=False)
        else:
...
def drawCondiTree(path, mode, level, limitCntRatio, prevResult):
    # data = df
    # mode = 'buy'
    # level = 3
    # limitCntRatio = 10
    ...
    for i in range(0, level + 1):
        ...
        if i == 0:
            ...
            ## Save pickle
            # with open('C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle2/lvl_0_1.pkl', 'wb') as f:
            ...
            for j in range(1, leaf + 1):
                if j % branch == 1:
                    ...
                    tmp = pd.DataFrame()
                    ## Load pickle
                    # with open("C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle2/lvl_{}_{}.pkl".format(i - 1,
                    #                                                                                       j // branch + 1),
                    #           "rb") as fr:
                    #     tmp = pickle.load(fr)
                    ...
                    print("[" + datetime.datetime.today().strftime(
                        "%Y-%m-%d %H:%M:%S") + "] Lvl_" + str(i) + " ------------- prevBcnt : " + str(
                    ...
                    # with open("C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle2/lvl_{}_{}.pkl".format(i, j - 1),
                    #           "rb") as fr:
                    #     tmp2 = pickle.load(fr)
                    if tmp2[0].shape[0] > 0:
                ...
                # with open('C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle2/lvl_{}_{}.pkl'.format(i, j), 'wb') as f:
                ...
                #             # if os.path.isfile(
                #             #         'C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle2/lvl_{}_{}.pkl'.format(i - 2,
                #             #                                                                                     j)):
                #             #     os.remove(
                #             #         'C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle2/lvl_{}_{}.pkl'.format(i - 2,
                #             #                                                                                     j))
                ...
                #         # if os.path.isfile(
                #         #         'C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle2/lvl_{}_{}.pkl'.format(i - 1,
                #         #                                                                                     j // branch)):
                #         #     os.remove('C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle2/lvl_{}_{}.pkl'.format(i - 1,
                #         #                                                                                           j // branch))
                #     except Exception as e:
                #         print(e)
                #         pass
                ...
                #         # if os.path.isfile('C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle2/lvl_{}_{}.pkl'.format(i,
                #         #                                                                                               j - branch + 1)):
                #         #     os.remove('C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle2/lvl_{}_{}.pkl'.format(i,
                #         #                                                                                           j - branch + 1))
                #     except Exception as e:
                #         pass
...
path = "C:/Users/Shine_anal/PycharmProjects/anlaysis/buyFind1.csv"
# df = pd.read_csv("C:/Users/Shine_anal/PycharmProjects/anlaysis/target.csv")

# function Exec
# fResult = drawCondiTree(df.iloc[:,0:50], 'buy', 2, 0.4)
fResult = drawCondiTree(path, 'buy', 1, 0.2, pd.DataFrame())
# fResult에 lvl1 조건들 제외하고 돌려 주는 것
fResult2 = drawCondiTree(path, 'buy', 4, 0.2, fResult)
