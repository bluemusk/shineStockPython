...
def makeMatrix(data, aggr_df, conData, leafType, rRatYn, rRatNum, rRtYn, rRtLvl, rRtNum, condition, datBool, lvl, ratDf, invYn, treeNm):
    ...
    # aggr_df = data.drop(tmpInv[8].index)['pur_gubn5']
    # conData = tmpInv[1]
    # leafType = 'B'
    # rRatYn = 'Y'
    # rRatNum = 1
...
def drawTree(data, level, limitRatio, initConds, rRatYn, rRatNum, rRtYn, rRtLvl, rRtNum, limitCntRatio):
    #####################################################################################
    #  트리를 그리는 로직
    # level = 2
    # data = newDff
                ...
                for j in range(1, leaf + 1):
                    ...
                    if j % trees.iloc[row].branch == 1:
                        ...
                        if prevDcnt > limitCnt:
                            ...
                            setattr(mod, 'lvl_{}_{}'.format(i, j),
                                    [pd.DataFrame(), pd.DataFrame(), 0, 0, 0, 0, tmpCon,
                    ...
                    # -----------------------------------------------------------------------------------------------
                        ...
                        if prevDcnt > limitCnt:
                            ...
                            tmpDat, tmpAggrDf, tmpRat, tmpBool, tmpCon, tmpConDf, tmpRatList = makeMatrix(
                                data.drop(tmpInv[8].index), data.drop(tmpInv[8].index)['pur_gubn5'],
                                tmpInv[1], 'D', rRatYn, rRatNum, rRtYn, rRtLvl,
                                rRtNum, tmpInvCon, ~tmpInv[0], i, '', 'Y', trees.iloc[row].treeNm)
                        else:
                            setattr(mod, 'lvl_{}_{}'.format(i, j),
                                    [pd.DataFrame(), pd.DataFrame(), 0, 0, 0, 0, tmpCon,
                    ...
                    try:
                        bcnt = int(tmpRat.bcnt)
                    except Exception as e:
                        bcnt = 0.8
                        pass

                    try:
                        dcnt = int(tmpRat.dcnt)
                    except Exception as e:
                        dcnt = 0.8
                        pass
                    ...
                    try:
                        fResultT = fResultT.append(
                            pd.DataFrame([('lvl_' + str(i) + '_' + str(j), tmpCon, bcnt, dcnt, bcnt / dcnt)],
...
dfCon = pd.read_csv('C:/Users/Shine_anal/PycharmProjects/anlaysis/kbuy1_con.csv', header=None)
...
# 실행 구문
fResult_md = drawTree(newDff, 2, 4, dfConF, 'Y', 1, 'Y', 3, 1, 1)
fResult_pm = drawTreePM(newDff, 7, 4, dfConF, 'Y', 1, 'Y', 3, 1)
fResult_pmn = drawTreePMN(newDff, 7, 4, dfConF, 'Y', 1, 'Y', 3, 1)
fResult_md = fResult_md.sort_values('dvsb', ascending=False)
...
fResult_md.to_csv("C:/Users/Shine_anal/PycharmProjects/anlaysis/invData.csv")


# data확인!!
fResult_md['condi'].iloc[0]
condition = 'dis20_ang1 <= 14.0 AND DIn_ang1_DIp_ang2 <= -15.0'
checkCondition(df, condition)

(high_low_rate <= 10.0) AND low_price_ang2 > 0.0 AND fast5K_fast10D > 61.0	288	72	4
((high_low_rate > 10.0) OR low_price_ang2 <= 0.0 OR fast5K_fast10D <= 61.0) AND fast10K_fast20K > 0.0	1872	320	5.85
(((high_low_rate <= 10.0) AND low_price_ang2 > 0.0 AND fast5K_fast10D > 61.0) OR fast10K_fast20K <= 0.0) AND momp_sig_momp5_sig4 > -9.0	6387	895	7.136312849


df[
      (df['high_low_rate'] <= 10 )
& (df['low_price_ang2'] > 0)
& (df['fast5K_fast10D'] > 61)
...
| (df['fast10K_fast20K'] <= 0 ))
& (df['momp_sig_momp5_sig4'] > -9)
].value_counts('pur_gubn5')
