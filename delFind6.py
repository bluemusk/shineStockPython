...
def custom_func(col, aggr_df):
    ...
    step1 = round((num_1 - num_0) / 5, 1)
    step2 = round((num_2 - num_1) / 5, 1)
    step3 = round((num_3 - num_2) / 5, 1)
    step4 = round((num_4 - num_3) / 5, 1)
...
def conditionMake(data, aggr_df, mode, leafType, lvl, condition, prevBcnt, prevDcnt, ratDf, exceptCon):
    ...
    try:
        ...
        #     tmpFinal = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.7)].sort_values('bvsd', ascending=False).iloc[3:]
        # elif leafType == 'B':
        #     tmpFinal = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.7)].sort_values('bvsd', ascending=False).iloc[4:]
        # elif leafType == 'C':
        #     tmpFinal = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.7)].sort_values('bvsd', ascending=False).iloc[5:]
        # elif leafType == 'D':
        #     tmpFinal = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.6)].sort_values('bvsd', ascending=False).iloc[0:]
        # elif leafType == 'E':
        #     tmpFinal = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.6)].sort_values('bvsd', ascending=False).iloc[1:]
        if leafType == 'A':
            # 이전 레벨 bcnt의 20% 이상이면서 bvsd가 가장 좋은 것
            tmpFinal = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.9)].sort_values('bvsd', ascending=False).iloc[2:]
        elif leafType == 'B':
            tmpFinal = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.8)].sort_values('bvsd', ascending=False).iloc[2:]
        elif leafType == 'C':
            tmpFinal = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.7)].sort_values('bvsd', ascending=False).iloc[2:]
        elif leafType == 'D':
            tmpFinal = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.6)].sort_values('bvsd', ascending=False).iloc[2:]
        elif leafType == 'E':
            tmpFinal = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.5)].sort_values('bvsd', ascending=False).iloc[2:]
        else:
            tmpFinal = pd.DataFrame()
...
def conditionMakeInit(data, aggr_df, mode, leafType, lvl, condition, prevBcnt, prevDcnt, ratDf, exceptCon):
    ...
    try:
        if leafType == 'A':
            # 이전 레벨 bcnt의 20% 이상이면서 bvsd가 가장 좋은 것
            tmpFinal = tmpFinal[(tmpFinal['dcnt'] <= prevDcnt * 0.6)].sort_values('bcnt', ascending=False).iloc[5:]
        elif leafType == 'B':
            tmpFinal = tmpFinal[(tmpFinal['dcnt'] <= prevDcnt * 0.6)].sort_values('bcnt', ascending=False).iloc[6:]
        elif leafType == 'C':
            tmpFinal = tmpFinal[(tmpFinal['dcnt'] <= prevDcnt * 0.6)].sort_values('bcnt', ascending=False).iloc[7:]
        elif leafType == 'D':
            tmpFinal = tmpFinal[(tmpFinal['dcnt'] <= prevDcnt * 0.6)].sort_values('bcnt', ascending=False).iloc[8:]
        elif leafType == 'E':
            tmpFinal = tmpFinal[(tmpFinal['dcnt'] <= prevDcnt * 0.6)].sort_values('bcnt', ascending=False).iloc[9:]
        else:
            tmpFinal = pd.DataFrame()
        # if leafType == 'A':
        #     # 이전 레벨 bcnt의 20% 이상이면서 bvsd가 가장 좋은 것
...
def drawCondiTree(data, level, limitCntRatio, initCondition, order, name):
    ...
    #     if data['RNN'].iloc[i] <= 10 and data['EARNING_RATE'].iloc[i] >= 0:
    ...
    for i in range(0, level + 1):
        ...
        if i == 0:
            ...
            with open('C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle/' + name + '/lvl_0_1.pkl', 'wb') as f:
                pickle.dump(['', '', data.iloc[:, 570],
                     pd.concat([data.iloc[:, 121:429], data.iloc[:, 457:569], data.iloc[:, 571:]], axis=1), initCondition,
                     ''], f)
            del (data)
        else:
            for j in range(1, leaf + 1):
                if j % branch == 1:
                    ...
                    tmp = pd.DataFrame()
                    ## Load pickle
                    with open("C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle/" + name + "/lvl_{}_{}.pkl".format(
                            i - 1,
                            j // branch + 1),
                              "rb") as fr:
                        tmp = pickle.load(fr)
                    ...
                    try:
                        prevDcnt = tmp[2].value_counts()[0]
                    except Exception as e:
                        prevDcnt = 0.8
                        pass

                    try:
                        prevBcnt = tmp[2].value_counts()[1]
                    except Exception as e:
                        prevBcnt = 0.8
                        pass
                ...
                try:
                    for x in range(0, len(tmp[3].columns)):
                        if tmpCondition[0:4] in tmp[3].columns[x]:
                            tmpNxtRat = tmpNxtRat[~(tmpNxtRat['condi'] == tmp[3].columns[x])]
                except Exception as e:
                    pass
                ...
                print("[" + str(order) + "][" + datetime.datetime.today().strftime(
                    ...
                    bcnt) + " / dcnt : " + str(dcnt) + " / condition : " + tmpCondition)
...
def drawCondiInitTree(path, level, limitCntRatio, additionalCond, name, label):
    ...
    if additionalCond != "":
        data = data[eval(additionalCond)]

    if label != 'all':
        data = data[data['LBL'] == label]
    ...
    for i in range(0, 2):
        ...
        if i == 0:
            setattr(mod, 'lvl_0_1',
                    ['', '', data.iloc[:, 570],
                     pd.concat([data.iloc[:, 121:429], data.iloc[:, 457:569], data.iloc[:, 571:]], axis=1), '',
                     ''])
        else:
            for j in range(1, leaf + 1):
                if j % branch == 1:
                    ...
                    tmp2 = getattr(mod, 'lvl_{}_{}'.format(i, j - 1))
                    if tmp2[0].shape[0] > 0:
                        tmpDat, tmpAggrDf, tmpBool, tmpFinal, tmpCondition, tmpRatList = conditionMakeInit(tmp[3],
                                                                                                           tmp[2], mode,
                                                                                                           'B', i,
                                                                                                           tmp[4],
                                                                                                           prevBcnt,
                ...
                setattr(mod, 'lvl_{}_{}'.format(i, j), [tmpFinal, tmpBool, tmpAggrDf, tmpDat, tmpCondition, tmpRatList])
                # tmpRatList[tmpRatList['bcnt'] >5000].sort_values('bvsd', ascending=False)
    ...
    for w in range(0, fResultT.shape[0]):
        ...
        # try:
        #     for p in range(0, len(tmpData.columns)):
        #         if fResultT.iloc[w]['condi'][0:4] in tmpData.columns[p]:
        #             tmpData = tmpData.drop(tmpData.columns[p], axis=1)
        #             print(tmpData.columns[p])
        # except Exception as e:
        #     pass

        finalReturn = finalReturn.append(
            drawCondiTree(tmpData, level, limitCntRatio, fResultT.iloc[w]['condition'], w, name))
        finalReturn.to_csv(path + "_result_" + str(w) + name + ".csv")
...
name = 'SELLMART_ALL_NEW_510_4_all'
# function Exec
fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'all')

# name = 'SELLMART_ALL_NEW_510_4_fsjtabuy'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'fsjtabuy')

# name = 'SELLMART_ALL_NEW_510_4_fncbuy3'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'fncbuy3')

# name = 'SELLMART_ALL_NEW_510_4_finobuy1'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'finobuy1')

# name = 'SELLMART_ALL_NEW_510_4_finobuy2'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'finobuy2')

# name = 'SELLMART_ALL_NEW_510_4_finobuyt8'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'finobuyt8')
# ##########################################################################################
#
...
name = 'SELLMART_ALL_NEW_1015_4_all'
# function Exec
fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'all')

# name = 'SELLMART_ALL_NEW_1015_4_fsjtabuy'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'fsjtabuy')

# name = 'SELLMART_ALL_NEW_1015_4_fncbuy3'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'fncbuy3')

# name = 'SELLMART_ALL_NEW_1015_4_finobuy1'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'finobuy1')

# name = 'SELLMART_ALL_NEW_1015_4_finobuy2'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'finobuy2')

# name = 'SELLMART_ALL_NEW_1015_4_finobuyt8'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'finobuyt8')
# ##########################################################################################
# # Data Loading
...
name = 'SELLMART_ALL_NEW_15AB_4_all'
# function Exec
fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'all')

# name = 'SELLMART_ALL_NEW_15AB_4_fsjtabuy'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'fsjtabuy')

# name = 'SELLMART_ALL_NEW_15AB_4_fncbuy3'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'fncbuy3')

# name = 'SELLMART_ALL_NEW_15AB_4_finobuy1'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'finobuy1')

# name = 'SELLMART_ALL_NEW_15AB_4_finobuy2'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'finobuy2')

# name = 'SELLMART_ALL_NEW_15AB_4_finobuyt8'
# # function Exec
# fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name, 'finobuyt8')
