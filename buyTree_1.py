    ...
    try:
        if leafType == 'A':
            ...
            if lvl == 1 or lvl == 2:
                ...
                tmpFinal = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.6) & (tmpFinal['bcnt']/tmpFinal['dcnt'] >= prevBcnt / prevDcnt * 1.4)].sort_values('bcnt', ascending=False)
            else:
            ...
            if lvl == 1 or lvl == 2 or lvl == 3:
                ...
                tmpFinal = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.3) & (tmpFinal['bcnt']/tmpFinal['dcnt'] >= prevBcnt / prevDcnt * 1.4)].sort_values('bvsd', ascending=False)
            else:
            ...
            if lvl == 1 or lvl == 2 or lvl == 3:
                ...
                tmpFinal = tmpFinal[(tmpFinal['dcnt'] < prevDcnt * 0.9) & (tmpFinal['bcnt']/tmpFinal['dcnt'] >= prevBcnt / prevDcnt * 1.4)].sort_values('bcnt', ascending=False)
            else:
                # 이전 레벨 bcnt의 60% 이상이면서 bvsd가 가장 좋은 것
...
def drawCondiInitTree(path, level, limitCntRatio, additionalCond, name):
    ...
    # data = data[data['high5d_low5d_rate'] > 5.6]
    branch = 5
    mode = 'buy'
    mod = sys.modules[__name__]
    fResultT = pd.DataFrame()
    ...
    finalReturn = finalReturn.append(drawCondiTree(data, level, limitCntRatio, '', 0, name))
    finalReturn.to_csv(path + "_result_" + ".csv")
...
fResultMid.to_csv("C:/Users/Shine_anal/Desktop/inodeltt_newcolm_close_h4_9year_202009_new_result.csv")
