    ...
    for u in range(branch * (lvlNum - 1) + 1, branch * lvlNum + 1):
        try:
            ...
            if u % branch == 1:
                ...
                # print(ray.get(tmp)[1])
                # print('----------------')
                # result = pd.concat([ray.get(tmp)[0],ray.get(tmp)[1],ray.get(tmp)[2],ray.get(tmp)[3],ray.get(tmp)[4],ray.get(tmp)[5]
                ...
                # result = colValueCalc(df_split[k], aggr_df)
                # print(result)
...
if __name__ == '__main__':
    ...
    realFinal.to_csv("C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle/RESULT/" + name + "_result.csv")
    fResultMid = makeFinalSet(path, name)
