...
def conditionMake(data, aggr_df, mode, leafType, lvl, condition, prevBcnt, prevDcnt, ratDf, exceptCon):
    ...
    try:
        if leafType == 'A':
            ...
            tmpFinal = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.3)].sort_values('bvsd', ascending=False)
        elif leafType == 'E':
...
def drawCondiTree(data, level, limitCntRatio, initCondition, order, name):
    ...
    for i in range(0, data.shape[0]):
        if data['RNN'].iloc[i] <= 25 and data['ERN'].iloc[i] <= 0:
    ...
    for i in range(0, level + 1):
        ...
        if i == 0:
            ...
            with open('C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle/' + name + '/lvl_0_1.pkl', 'wb') as f:
                pickle.dump(['', '', data.iloc[:, -1], data.iloc[:,120:], initCondition, ''], f)
            del (data)
        else:
...
def drawCondiInitTree(path, level, limitCntRatio, additionalCond, name):
    ...
    for i in range(0, 2):
        leaf = branch ** i
        if i == 0:
            setattr(mod, 'lvl_0_1', ['', '', data.iloc[:, -1], data.iloc[:,120:], '', ''])
        else:
            for j in range(1, leaf + 1):
                if j % branch == 1:
                    tmpNxtRat = pd.DataFrame()
...
name = 'TRADE_BUYMART3_202103_202107'

# Data Loading
path = "C:/Users/Shine_anal/PycharmProjects/anlaysis/TRADE_BUYMART3_202103_202107.csv"
additionalCond = ''

# function Exec
fResult = drawCondiInitTree(path, 4, 0.2, additionalCond, name)
