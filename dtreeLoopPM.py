...
def makeFinalSet(path, name, lastRatio, initData, limitCnt):
    ...
    file_list_rslt = [file for file in lists if file.endswith("_result.csv") and file.find(name) > 0]
    ...
    # limitCnt = 10
    fResultMid = fResultMid[fResultMid['dcnt'] >= limitCnt]
            ...
            if treeNm == 'tree1':
                if u % branch == 1:
                    ...
                    if lvl == 1:
                        ...
                        tmpExec = tmpFinal[(tmpInvFinal['bcnt'] < tmpInvFinal['prevBcnt'] * 0.8)].sort_values('dvsb', ascending=False).iloc[0]
                ...
                if u % branch == 1:
                    ...
                    if lvl == 1:
                        ...
                        tmpExec = tmpFinal[tmpFinal['bcnt'] + tmpFinal['dcnt']
                                       > (tmpFinal['prevBcnt'] + tmpFinal['prevDcnt']) * 0.4].sort_values('dvsb', ascending=False).iloc[0]
                    ...
                    tmpExec = tmpFinal[tmpFinal['bcnt'] < tmpFinal['prevBcnt'] * 0.9 \
                ...
                if u % branch == 1:
                    ...
                    firstBcnt, firstDcnt, tmpZeroCnt = 0, 0, 0
                    if lvl == 1:
                        tmpExec = tmpFinal[tmpFinal['dvsb'] > (tmpFinal['prevDcnt'] / tmpFinal['prevBcnt']) * 1.04 \
                    ...
                    tmpExec = tmpFinal[tmpFinal['dvsb'] > (tmpFinal['prevDcnt'] / tmpFinal['prevBcnt'])].sort_values('dcnt', ascending=False).iloc[0]
            elif treeNm == 'tree4':
                if u % branch == 1:
                    tmpFinal = splitData(data, aggr_df, prevBcnt, prevDcnt, limitCnt)
                    firstBcnt, firstDcnt, tmpZeroCnt = 0, 0, 0
                    ...
                    tmpExec = tmpFinal[tmpFinal['dvsb'] > (tmpFinal['prevDcnt'] / tmpFinal['prevBcnt'])].sort_values('sRt', ascending=False).iloc[0]
            elif treeNm == 'tree5':
                if u % branch == 1:
                    tmpFinal = splitData(data, aggr_df, prevBcnt, prevDcnt, limitCnt)
                    firstBcnt, firstDcnt, tmpZeroCnt = 0, 0, 0
                    if lvl == 1:
                        ...
                        tmpExec = tmpFinal[tmpFinal['dvsb'] > (tmpFinal['prevDcnt'] / tmpFinal['prevBcnt']) \
                                       ].sort_values('rRat', ascending=False).iloc[0]
                    ...
                    tmpExec = tmpFinal[tmpFinal['dvsb'] > (tmpFinal['prevDcnt'] / tmpFinal['prevBcnt'])].sort_values('rRt', ascending=False).iloc[0]
            elif treeNm == 'tree6':
                if u % branch == 1:
                    tmpFinal = splitData(data, aggr_df, prevBcnt, prevDcnt, limitCnt)
                    firstBcnt, firstDcnt, tmpZeroCnt = 0, 0, 0
                ...
                if u % branch == 1:
                    ...
                    if lvl == 1:
                        ...
                        tmpExec = tmpFinal[tmpFinal['dvsb'] > (tmpFinal['prevDcnt'] / tmpFinal['prevBcnt']) \
                                       ].sort_values('bDc', ascending=False).iloc[0]
                    ...
                    tmpExec = tmpFinal[tmpFinal['dvsb'] > (tmpFinal['prevDcnt'] / tmpFinal['prevBcnt'])].sort_values('fRt', ascending=False).iloc[0]
            elif treeNm == 'tree8':
                if u % branch == 1:
                    tmpFinal = splitData(data, aggr_df, prevBcnt, prevDcnt, limitCnt)
                    firstBcnt, firstDcnt, tmpZeroCnt = 0, 0, 0
                    if lvl == 1:
                        ...
                        tmpExec = tmpFinal[tmpFinal['bcnt'] < tmpFinal['prevBcnt'] \
                                       ].sort_values('dDc').iloc[0]
                    ...
                    tmpExec = tmpFinal[tmpFinal['dvsb'] > (tmpFinal['prevDcnt'] / tmpFinal['prevBcnt'])].sort_values('rGr', ascending=False).iloc[0]
...
def makeLevel(vLoop, paramLevel, paramLastRatio, limitCnt, name, data, initCond, lastYn, paramMemoryYn):
    ...
    for row in range(0, trees.shape[0]):
            ...
            if i == 0:
                ...
                for j in range(1, leaf + 1):
                    ...
                    try:
                        ...
                        print(
                            '######################################################################################################################################')
                        print("[Loop - " + str(vLoop) + "][" + name + "][PM][" + trees.iloc[
        ...
        try:
            tmpMkL.to_csv(
                "C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle/RESULTNEW/[Loop - " + str(vLoop) + "][PM][" +
