...
def conditionMake(data, aggr_df, mode, leafType, lvl, condition, prevBcnt, prevDcnt, ratDf, exceptCon):
    ...
    try:
        if leafType == 'A':
            ...
            tmpFinal = tmpFinal[(tmpFinal['diffRat'] > 0.8) & (tmpFinal['diffRat'] < 4)].sort_values('bvsd', ascending=False)
        elif leafType == 'B':
            # 이전 레벨 대비 dcnt를 20% 이상 감소시키고 이전 레벨 비율보다 좋아지게 하는 조건
            tmpFinal = tmpFinal[(tmpFinal['diffRat'] > 0.8) & (tmpFinal['diffRat'] < 2) & (tmpFinal['dcntDiff'] > prevDcnt * 0.2)].sort_values('bvsd', ascending=False)
        elif leafType == 'C':
            # 전 레벨 대비 지우는 비율(bcnt/dcnt)가 0.8에서 3 사이인 녀석 중에 dcnt를 가장 많이 지우는 조건
            tmpFinal = tmpFinal[(tmpFinal['diffRat'] > 0.8) & (tmpFinal['diffRat'] < 3)].sort_values('bvsd', ascending=False)
        elif leafType == 'D':
            ...
            if lvl == 1 or lvl == 2 or lvl == 3:
                ...
                tmpFinal = tmpFinal[(tmpFinal['dcntDiff'] > prevDcnt * 0.1)].sort_values('dcntDiff', ascending=False)
        else:
...
def drawCondiTree(path, mode, level, limitCntRatio, prevResult):
    ...
    if prevResult.shape[0] > 0 != '':
        # lvl1의 조건들
        expCondi = fResult[(prevResult['lvl'] == 'lvl_1_1') |
                           (prevResult['lvl'] == 'lvl_1_2') |
                           (prevResult['lvl'] == 'lvl_1_3') |
                           (prevResult['lvl'] == 'lvl_1_4') |
                           (prevResult['lvl'] == 'lvl_1_5')].condi

        # 원천 데이터에서 lvl1조건들을 빼준다.
        data = pd.read_csv(path)
        data = exceptColumn(data, expCondi)
    else:
        data = pd.read_csv(path)
    branch = 5
...
def drawCondiInitTree(path, initCnt):
    data = pd.read_csv(path)
    branch = 5
