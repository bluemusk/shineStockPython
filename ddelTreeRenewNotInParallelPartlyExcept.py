    ...
    for u in range(branch * (lvlNum - 1) + 1, branch * lvlNum + 1):
        try:
            ...
            if u % branch == 1:
                ...
                # 이전 레벨 bcnt 40%이상 감소, 이전 dvsb보다 향상, dcnt가 가장 많은 것 not in
                tmpExec = tmpFinal[(tmpFinal['bcnt'] <= prevBcnt * 0.6)
                                   & (tmpFinal['dcnt'] / tmpFinal['bcnt'] > prevDcnt / prevBcnt * 1.2)
                                   ].sort_values('dcnt', ascending=False).iloc[0]
                ...
                # rRt상위 60% 중에서 dcnt가 가장 많은 것 not in
                tmpExec = tmpFinal[(tmpFinal['rRt'] > tmpFinal['rRt'].median())
                                   & (tmpFinal['dcnt'] / tmpFinal['bcnt'] > prevDcnt / prevBcnt * 1.2)
                                   ].sort_values('dcnt', ascending=False).iloc[0]
                ...
                # rRt가 이전레벨 dvsb보다 크면 sRt가 가장 큰것
                # 아니면 rRat > 20 인 것 중에 rRt가 가장 큰것  not in
                tmpExec = tmpFinal[(tmpFinal['rRat'] >= 20)
                                   & (tmpFinal['dcnt'] / tmpFinal['bcnt'] > prevDcnt / prevBcnt * 1.2)
                                   ].sort_values('rRt', ascending=False).iloc[0]
            ...
            if len(tmpExec) > 0:
                ...
                if u % branch == 14 or u % branch == 5 or u % branch == 9 or u % branch == 11:
                    conHI = tmpExec['condi']
                    valHI = float(tmpExec['value'])
                    calHI = tmpExec['cal']

                if u % branch == 0 or u % branch == 6 or u % branch == 10 or u % branch == 12:
                    conH = conHI
                    valH = valHI
                    if calHI == 'GT':
                        calH = 'LT'
                ...
                print("[" + datetime.datetime.today().strftime("%Y-%m-%d %H:%M:%S")
                      + '] lvl'
                      + str(lvl) + "_" + str(u)
                      + "\ndvsb : " + str(round(tmpDcnt / tmpBcnt, 2))
                      + " / bcnt : " + str(int(tmpBcnt))
                      + " / dcnt : " + str(int(tmpDcnt))
                      + " / condition : " + condition)
                ...
                try:
                    fResultT = fResultT.append(
                        pd.DataFrame([('lvl_' + str(lvl) + '_' + str(u), condition, str(int(tmpBcnt)),
                                       str(int(tmpDcnt)), tmpBcnt / tmpDcnt,
                                       tmpDcnt / tmpBcnt)],
