...
def conditionMake(data, aggr_df, lvl, initCondition, prevBcnt, prevDcnt, branch, name, limitLvl, lvlNum, lastLimitRatio,
    ...
    if mode == 'init':
        ...
        tmpExec = tmpFinal[(tmpFinal['dcnt'] >= prevDcnt * 0.8)
                         & (tmpFinal['bcnt'] <= prevBcnt * 0.8)].sort_values('dvsb', ascending=False)
        ...
        for u in range(branch * (lvlNum - 1) + 1, branch * lvlNum + 1):
            try:
                ...
                if u % branch == 1:
                    ...
                    # 이전 레벨 bcnt의 20% 이상이면서 bvsd가 가장 좋은 것
                    tmpExec = tmpFinal[(tmpFinal['bcnt'] >= 0) & (tmpFinal['dcnt'] >= prevDcnt * 0.2) & (
                                tmpFinal['bcnt'] < prevBcnt * 0.7) & (
                                                    tmpFinal['dcnt'] / tmpFinal['bcnt'] > prevDcnt / prevBcnt)].sort_values(
                        'dvsb', ascending=False).iloc[0]
                elif u % branch == 2:
                    # 이전 레벨 dcnt의 70% 미만 & 이전레벨의 전체건수의 20% 이상 이면서 bvsd가 가장 좋은 것
                    tmpExec = tmpFinal[(tmpFinal['bcnt'] >= 0) & (tmpFinal['dcnt'] >= prevDcnt * 0.4) & (
                                tmpFinal['bcnt'] < prevBcnt * 0.6) & (
                                                    tmpFinal['dcnt'] / tmpFinal['bcnt'] > prevDcnt / prevBcnt)].sort_values(
                        'dvsb',
                    ...
                    if lvl == 1 or lvl == 2:
                        ...
                        tmpExec = tmpFinal[(tmpFinal['bcnt'] < prevBcnt * 0.6) & (tmpFinal['dcnt'] >= prevDcnt * 0.3) & (
                                    tmpFinal['dcnt'] / tmpFinal['bcnt'] > prevDcnt / prevBcnt)].sort_values('dcnt',
                                                                                                            ascending=False).iloc[0]
                    else:
                        # 이전 레벨 bcnt의 60% 이상이면서 bvsd가 가장 좋은 것
                        tmpExec = tmpFinal[(tmpFinal['bcnt'] >= 0) & (tmpFinal['dcnt'] >= prevDcnt * 0.3) & (
                    ...
                    if lvl == 1 or lvl == 2 or lvl == 3:
                        ...
                        tmpExec = tmpFinal[(tmpFinal['bcnt'] < prevBcnt * 0.4) & (
                                    tmpFinal['dcnt'] / tmpFinal['bcnt'] > prevDcnt / prevBcnt)].sort_values('dcnt',
                                                                                                            ascending=False).iloc[0]
                    else:
                    ...
                    if lvl == 1 or lvl == 2:
                        ...
                        # 이전 레벨 bcnt의 60% 이상이면서 bvsd가 가장 좋은 것
                        tmpExec = tmpFinal[(tmpFinal['bcnt'] >= 0) & (tmpFinal['dcnt'] >= prevDcnt * 0.1) & (
                                tmpFinal['dcnt'] / tmpFinal['bcnt'] > prevDcnt / prevBcnt)].sort_values('dvsb',
                                                                                                        ascending=False).iloc[0]
...
def makeFinalSet(path, name):
    # 결과 합치기
    fResultMid = pd.DataFrame()  # 결과 데이터프레임 선언
    lists = os.listdir("C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle/RESULT/")  # 결과 폴더 내에 파일 리스트 읽어오기
    file_list_rslt = [file for file in lists if
                      file.startswith(name + "_result_")]  # 즁간파일명이 result_original로 시작하는 것들만 취합

    for i in range(0, len(file_list_rslt)):
        print(file_list_rslt[i])
        tmp = pd.read_csv("C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle/RESULT/" + file_list_rslt[i])
        os.remove("C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle/RESULT/" + file_list_rslt[i])
    ...
    fResultMid.to_csv(path + name + "_ddelTreeParallel_result.csv")
...
if __name__ == '__main__':
    ...
    print('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
    print('최종결과파일은 ' + path + name + '_ddelTreeParallel_result.csv  입니다.')
    print('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
    #####################################################################################################
