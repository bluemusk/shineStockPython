...
def makeFinalSet(path, name):
    ...
    file_list_rslt = [file for file in lists if file.startswith(name)]

    fResultMid = pd.DataFrame()
    for i in range(0, len(file_list_rslt)):
        print(path + file_list_rslt[i])
...
if __name__ == '__main__':
    ...
    initBcnt = initData.iloc[:, 4].value_counts()[1]
    initDcnt = initData.iloc[:, 4].value_counts()[0]
    ...
    for y in range(0, 5):
        for u in range(1, 6):
            try:
                loopData = getattr(mod, 'dat{}'.format(str(y * 5 + u - 1)))

                if len(loopData) > 0:
                    dfRatio = splitData(loopData
                                     , loopData['pur_gubn5']
                                     , loopData['pur_gubn5'].value_counts()[1]
                                     , loopData['pur_gubn5'].value_counts()[0]
                                        )
                    print('---------------------initCondition - ' + str(y * 5 + u) + ' / bcnt : ' + str(loopData['pur_gubn5'].value_counts()[1]) + ' / dcnt : ' + str(loopData['pur_gubn5'].value_counts()[0]))
                    # if u == 1:
                    #     tmpExec = dfRatio[(dfRatio['dcnt'] <= (loopData['pur_gubn5'].value_counts()[0]) * 0.95)
                    #                     & (dfRatio['dcnt'] >= limitCnt)
                    #                     & (dfRatio['dcnt'] + dfRatio['bcnt'] >= (loopData['pur_gubn5'].value_counts()[1] + loopData['pur_gubn5'].value_counts()[0]) * 0.9)
                    #                     ].sort_values('dvsb', ascending=False).iloc[0]
                    # elif u == 2:
                    #     tmpExec = dfRatio[(dfRatio['dcnt'] + dfRatio['bcnt'] <= (loopData['pur_gubn5'].value_counts()[1] + loopData['pur_gubn5'].value_counts()[0]) * 0.97)
                    #                     & (dfRatio['dcnt'] >= limitCnt)
                    #                     & (dfRatio['dcnt'] + dfRatio['bcnt'] >= (loopData['pur_gubn5'].value_counts()[1] + loopData['pur_gubn5'].value_counts()[0]) * 0.9)
                    #                     ].sort_values('dvsb', ascending=False).iloc[0]
                    # elif u == 3:
                    #     tmpExec = dfRatio[(dfRatio['bcnt'] <= loopData['pur_gubn5'].value_counts()[1] * 0.98)
                    #                     & (dfRatio['dcnt'] >= limitCnt)
                    #                     & (dfRatio['dcnt'] + dfRatio['bcnt'] >= (loopData['pur_gubn5'].value_counts()[1] + loopData['pur_gubn5'].value_counts()[0]) * 0.9)
                    #                     ].sort_values('dcnt', ascending=False).iloc[0]
                    # elif u == 4:
                    #     tmpExec = dfRatio[(dfRatio['dcnt'] >= limitCnt)
                    #                     & (dfRatio['bcnt'] >= (loopData['pur_gubn5'].value_counts()[1]) * 0.9)
                    #                     ].sort_values('bcnt').iloc[0]
                    # elif u == 5:
                    #     tmpExec = dfRatio[(dfRatio['rRat'] >= 20)
                    #                     & (dfRatio['dcnt'] >= limitCnt)
                    #                     & (dfRatio['dcnt'] + dfRatio['bcnt'] >= (loopData['pur_gubn5'].value_counts()[1] + loopData['pur_gubn5'].value_counts()[0]) * 0.9)
                    #                     ].sort_values('rRt', ascending=False).iloc[0]
                    ...
                    if if_float(valH):
                        if calH == 'GT':
                            # tmpBool = (loopData['pur_gubn5'] == 0) & (loopData[conH] > valH)
                            tmpBool = loopData[conH] > valH
                        else:
                            # tmpBool = (loopData['pur_gubn5'] == 0) & (loopData[conH] <= valH)
                            tmpBool = loopData[conH] <= valH
                    else:
                        if calH == 'GT':
                            # tmpBool = (loopData['pur_gubn5'] == 0) & (loopData[conH]) > loopData[valH]
                    ...
                    setattr(mod, 'dat' + str(y * 5 + u), loopData.drop(conH, axis=1)[~tmpBool])

                    setattr(mod, 'datO' + str(y * 5 + u), initData.drop(conH, axis=1))

                else:
                    setattr(mod, 'cond' + str(y * 5 + u), '')
                    setattr(mod, 'dat' + str(y * 5 + u), pd.DataFrame())
                    setattr(mod, 'datO' + str(y * 5 + u), pd.DataFrame())
            except Exception as e:
                setattr(mod, 'cond' + str(y * 5 + u), '')
                setattr(mod, 'dat' + str(y * 5 + u), pd.DataFrame())
                setattr(mod, 'datO' + str(y * 5 + u), pd.DataFrame())
    ...
    for x in range(1, 27):
        ...
        for i in range(0, paramLevel + 1):
            if i == 0:
                ...
                with open('C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle/' + name + 'New' + '/lvl_0_1.pkl', 'wb') as f:
                    pickle.dump(['', '', data.iloc[:, 4], data, initCond, ''], f)
                ...
                for j in range(1, leaf + 1):
                    ...
                    try:
                        ...
                        print("====================================================================")
                        print("[" + str(x) + "][" + datetime.datetime.today().strftime(
        ...
        realFinal.to_csv("C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle/RESULTNEW/" + name + "_result" + str(x) + ".csv")
    fResultMid = makeFinalSet(path, name)

    #####################################################################################################
    # 최종결과파일명   사용자 지정 이름_allnew_ddelTreeNew_result.csv
    print('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
    print('최종결과파일은 ' + path + name + '_dtreeInitialCondition_result.csv  입니다.')
    print('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
    #####################################################################################################
