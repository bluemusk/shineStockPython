    ...
    for u in range(branch * (lvlNum - 1) + 1, branch * lvlNum + 1):
        try:
            ...
            if u % branch == 1:
                ...
                tmpExec = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.7) & (tmpFinal['dcnt'] <= prevDcnt * 0.7) & (tmpFinal['bcnt'] / tmpFinal['dcnt'] > prevBcnt / prevDcnt * 1.4)].sort_values('bvsd', ascending=False).iloc[0]
            elif u % branch == 2:
                tmpExec = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.7) & (tmpFinal['dcnt'] <= prevDcnt * 0.65) & (tmpFinal['bcnt'] / tmpFinal['dcnt'] > prevBcnt / prevDcnt * 1.3)].sort_values('bvsd', ascending=False).iloc[1]
            elif u % branch == 3:
                tmpExec = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.6) & (tmpFinal['dcnt'] <= prevDcnt * 0.63) & (tmpFinal['bcnt'] / tmpFinal['dcnt'] > prevBcnt / prevDcnt * 1.25)].sort_values('bvsd', ascending=False).iloc[0]
            elif u % branch == 4:
                tmpExec = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.6) & (tmpFinal['dcnt'] <= prevDcnt * 0.58) & (tmpFinal['bcnt'] / tmpFinal['dcnt'] > prevBcnt / prevDcnt * 1.2)].sort_values('bvsd', ascending=False).iloc[1]
            elif u % branch == 5:
                tmpExec = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.5) & (tmpFinal['dcnt'] <= prevDcnt * 0.7) & (tmpFinal['bcnt'] / tmpFinal['dcnt'] > prevBcnt / prevDcnt * 1.21)].sort_values('bvsd', ascending=False).iloc[0]
            elif u % branch == 6:
                tmpExec = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.5) & (tmpFinal['dcnt'] <= prevDcnt * 0.6) & (tmpFinal['bcnt'] / tmpFinal['dcnt'] > prevBcnt / prevDcnt * 1.22)].sort_values('bvsd', ascending=False).iloc[1]
            elif u % branch == 7:
                tmpExec = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.4) & (tmpFinal['dcnt'] <= prevDcnt * 0.57) & (tmpFinal['bcnt'] / tmpFinal['dcnt'] > prevBcnt / prevDcnt * 1.23)].sort_values('bvsd', ascending=False).iloc[0]
            elif u % branch == 0:
                tmpExec = tmpFinal[(tmpFinal['bcnt'] >= prevBcnt * 0.4) & (tmpFinal['dcnt'] <= prevDcnt * 0.54) & (tmpFinal['bcnt'] / tmpFinal['dcnt'] > prevBcnt / prevDcnt * 1.24)].sort_values('bvsd', ascending=False).iloc[1]
    ...
    for i in range(0, paramLevel + 1):
        if i == 0:
            ...
            with open('C:/Users/Shine_anal/PycharmProjects/anlaysis/pickle/' + name + 'ReNew' + '/lvl_0_1.pkl', 'wb') as f:
                pickle.dump(['', '', data['TGT'], pd.concat([pd.DataFrame(data['TGT']), data.iloc[:,81:]], axis=1), '', ''], f)
